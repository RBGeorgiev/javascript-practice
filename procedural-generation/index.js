...
import {
    ...
    terrainHeightIncreaseWeightSpan,
    terrainHeightIncreaseWeightInput,
    chanceForLandSpan,
    chanceForLandInput,
...
class MapGenerator {
    constructor(numOfPoints, seed = null) {
        ...
        this.terrainHeightIncreaseWeight = 120; // important value
    ...
    defineTilesHeight = () => {
        ...
        while (queue.length) {
            decrement = this.randRange(this.terrainHeightUniformity, this.terrainHeightIncreaseWeight) / 100;
...
function displayBiomeCountInHtml() {
    ...
    sortedBiomes
        ...
        .forEach((biome =>
            biomeCountDiv.innerHTML += `<span class="biomeCountEntry" data-biome="${biome[0]}"><span class="biomeCountBiome">${formatBiomeString(biome[0])}:</span> <span class="biomeCountValue">${biome[1].length}</span></span>`
...
function updateHtmlDisplayedValues() {
    ...
    for (let entry of biomeCountEntries) {
        entry.onclick = () => {
            ...
            mapGen.voronoi.renderCell(mapGen.biomeCount[biome][0], ctx2);
...
terrainHeightIncreaseWeightInput.oninput = (e) => {
    let val = +e.target.value;
    terrainHeightIncreaseWeightSpan.innerText = val;

    mapGen.terrainHeightIncreaseWeight = val;
    mapGen.changeMapTerrain();
